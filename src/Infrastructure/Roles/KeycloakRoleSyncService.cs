...
public sealed class KeycloakRoleSyncService : IKeycloakRoleSyncService
{
    ...
    private async Task<HttpClient> GetAuthorizedClientAsync(CancellationToken ct)
    {
        ...
        if (_options.UseAdminCredentials)
        {
            // Password grant na master realmu — za lokalni razvoj kad service account
            // još nema manage-realm rolu na Keycloak instancei.
        ...
        {
            // Client credentials — service account mora imati manage-realm rolu.
            form = new FormUrlEncodedContent(new Dictionary<string, string>
            {
            ...
            tokenPath = $"/realms/{_options.Realm}/protocol/openid-connect/token";
        }
