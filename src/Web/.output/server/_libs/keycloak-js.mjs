//#region node_modules/.pnpm/keycloak-js@26.2.4/node_modules/keycloak-js/lib/keycloak.js
/**
...
var Keycloak = class {
	...
	*/
	init = async (initOptions = {}) => {
		...
		if (typeof initOptions.adapter === "string" && [
			...
			"cordova",
			"cordova-native"
		].includes(initOptions.adapter)) this.#adapter = this.#loadAdapter(initOptions.adapter);
	...
	#loadDefaultAdapter() {
		...
		return {
			...
			logout: async (options) => {
				if ((options?.logoutMethod ?? this.logoutMethod) === "GET") {
	...
	*/
	async #loadOidcConfig(oidcProvider) {
		if (typeof oidcProvider === "string") {
			const openIdConfig = await fetchOpenIdConfig(`${stripTrailingSlash(oidcProvider)}/.well-known/openid-configuration`);
	...
	*/
	async #check3pCookiesSupported() {
		...
		return await applyTimeoutToPromise(new Promise((resolve) => {
			/**
			...
			*/
			const messageCallback = (event) => {
				...
				if (event.data !== "supported" && event.data !== "unsupported") return;
					...
					this.#loginIframe.enable = false;
					if (this.silentCheckSsoFallback) this.silentCheckSsoRedirectUri = void 0;
				}
		...
		}), this.messageReceiveTimeout, "Timeout when waiting for 3rd party check iframe message.");
	}
	/**
	* @param {KeycloakInitOptions} initOptions
	* @returns {Promise<void>}
	*/
	async #processInit(initOptions) {
			...
			switch (initOptions.onLoad) {
				case "check-sso":
					if (this.#loginIframe.enable) {
						await this.#setupCheckLoginIframe();
						if (!await this.#checkLoginIframe()) this.silentCheckSsoRedirectUri ? await this.#checkSsoSilently() : await doLogin(false);
		...
		if (initOptions.token && initOptions.refreshToken) {
			...
			if (this.#loginIframe.enable) {
				await this.#setupCheckLoginIframe();
				if (await this.#checkLoginIframe()) {
					this.onAuthSuccess?.();
	...
	*/
	...
	async #processCallback(oauth) {
		...
		*/
		...
		if (this.flow !== "implicit" && code) try {
			const response = await fetchAccessToken(this.endpoints.token(), code, this.clientId, oauth.redirectUri, oauth.pkceCodeVerifier);
	...
	async #scheduleCheckIframe() {
		if (this.#loginIframe.enable && this.token) {
			...
			if (await this.#checkLoginIframe()) await this.#scheduleCheckIframe();
		}
	}
	/**
	...
	*/
	createLoginUrl = async (options) => {
		...
		if (options?.acr) params.append("claims", buildClaimsParameter(options.acr));
	...
	*/
	createLogoutUrl = (options) => {
		...
		const params = new URLSearchParams([["client_id", this.clientId], ["post_logout_redirect_uri", this.#adapter.redirectUri(options)]]);
	...
	*/
	createAccountUrl = (options) => {
		...
		return `${url}/account?${new URLSearchParams([["referrer", this.clientId], ["referrer_uri", this.#adapter.redirectUri(options)]]).toString()}`;
	};
	/**
	* @returns {Promise<void>}
	*/
	...
	*/
	loadUserProfile = async () => {
		...
		/** @type {KeycloakProfile} */
		const profile = await fetchJSON(`${realmUrl}/account`, { headers: [buildAuthorizationHeader(this.token)] });
	...
	*/
	loadUserInfo = async () => {
		/** @type {{}} */
		const userInfo = await fetchJSON(this.endpoints.userinfo(), { headers: [buildAuthorizationHeader(this.token)] });
	...
	*/
	updateToken = async (minValidity) => {
		...
		if (this.#refreshQueue.length === 1) {
			...
			let timeLocal = (/* @__PURE__ */ new Date()).getTime();
			try {
				const response = await fetchRefreshToken(url, this.refreshToken, this.clientId);
				...
				if (error instanceof NetworkError && error.response.status === 400) this.clearToken();
...
async function generatePkceChallenge(pkceMethod, codeVerifier) {
	...
	return bytesToBase64(new Uint8Array(await sha256Digest(codeVerifier))).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
}
/**
* @param {number} len
...
async function fetchJSON(url, init = {}) {
	...
	return await (await fetchWithErrorHandling(url, {
		...init,
		headers
	})).json();
}
