...
function newTile(index: number, t: (key: string) => string): BentoTile {
  return {
    id: `tile-${index}`,
  ...
  return (
    <div className="rounded-sm border border-border-default">
      <fieldset className="border-b border-border-subtle p-4">
        <legend className="text-eyebrow mb-3 text-text-tertiary">
          {t("bentoBuilder.blueprintLegend")}
        </legend>
            ...
            <SelectContent>
              {gapOptions.map((option) => (
                <SelectItem key={option} value={option}>
                  {
                    t(
                      `bentoBuilder.gap${option.charAt(0).toUpperCase()}${option.slice(1)}` as never,
                    ) as string
                  }
                </SelectItem>
              ))}
            ...
            <SelectContent>
              {densityOptions.map((option) => (
                <SelectItem key={option} value={option}>
                  {
                    t(
                      `bentoBuilder.density${option.charAt(0).toUpperCase()}${option.slice(1)}` as never,
                    ) as string
                  }
                </SelectItem>
              ))}
            ...
            >
              <div className="mb-3 flex flex-wrap items-center gap-1.5">
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    patchTile(tile.id, { span: nextInCycle(bentoSpanOptions, tile.span) })
                  }
                >
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    patchTile(tile.id, { rowSpan: nextInCycle(bentoRowSpanOptions, tile.rowSpan) })
                  }
                >
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    patchTile(tile.id, { tone: nextInCycle(bentoToneOptions, tile.tone) })
                  }
                >
                  {tile.tone}
                </Button>
                ...
                <Button
                  ...
                  onClick={() =>
                    setTiles((current) => current.filter((entry) => entry.id !== tile.id))
                  }
                >
