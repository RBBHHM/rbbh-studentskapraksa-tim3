...
function getSideNavSections(t: (key: string) => string): readonly SideNavSection[] {
  return [
    {
      id: "operate",
      ...
      items: [
        ...
        {
          id: "accounts",
          label: t("navigationSection.sideNav.sections.accounts"),
          icon: Building2,
          badge: "24",
        },
        {
          id: "payments",
          label: t("navigationSection.sideNav.sections.payments"),
          icon: CreditCard,
        },
        {
          id: "lending",
          ...
          items: [
            {
              id: "lending-pipeline",
              label: t("navigationSection.sideNav.sections.lendingPipeline"),
            },
            { id: "lending-limits", label: t("navigationSection.sideNav.sections.lendingLimits") },
            {
              id: "lending-collateral",
              label: t("navigationSection.sideNav.sections.lendingCollateral"),
            },
          ],
        },
      ],
    },
    {
      ...
      items: [
        ...
        {
          id: "compliance",
          label: t("navigationSection.sideNav.sections.compliance"),
          icon: ShieldCheck,
          badge: "3",
        },
    ...
    >
      <div className="flex w-full flex-col gap-4 sm:flex-row">
          ...
          header={
            <div className="min-w-0">
              <p className="truncate text-sm font-bold text-text-primary">
                {t("navigationSection.sideNav.headerTitle")}
              </p>
              <p className="truncate text-xs text-text-tertiary">
                {t("navigationSection.sideNav.headerSubtitle")}
              </p>
            </div>
          }
          footer={
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-text-primary">
                  {t("navigationSection.sideNav.footerName")}
                </p>
                <p className="truncate text-xs text-text-tertiary">
                  {t("navigationSection.sideNav.footerRole")}
                </p>
              </div>
            </div>
          }
        ...
        <div className="min-w-0 flex-1 rounded-sm border border-border-subtle bg-surface-subtle p-6">
          ...
          >
            {collapsed
              ? t("navigationSection.sideNav.expandRail")
              : t("navigationSection.sideNav.collapseToIcons")}
          </Button>
        </div>
      </div>
    </Example>
            ...
            <BreadcrumbList>
              <BreadcrumbItem>
                <BreadcrumbLink href="#navigation">
                  {t("navigationSection.breadcrumb.overview")}
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink href="#navigation">
                  {t("navigationSection.breadcrumb.corporateBanking")}
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
  ...
  <Stat variant="plain" label="Since" value="1927" />
        ...
        >
          <StatGroup
            columns={4}
            gap="default"
            label={t("statisticsSection.portfolioLabel")}
            className="w-full"
          >
            <Stat
              variant="rule"
              size="sm"
              label={t("statisticsSection.markets")}
              value="17"
              hint={t("statisticsSection.marketsHint")}
            />
            <Stat
              variant="card"
              size="sm"
            ...
            <Stat
              variant="plain"
              size="sm"
              label={t("statisticsSection.since")}
              value="1927"
              hint={t("statisticsSection.sinceHint")}
            />
          </StatGroup>
        </Example>
        ...
        >
          <div className="space-y-4">
            {(["subtle", "brand", "corporate", "inverse"] as const).map((surface) => (
              <Example
                key={surface}
                title={t("sections.surfaces.exampleTitle", { surface }) as string}
                surface={surface}
                className="block"
              >
                <StatGroup
                  columns={3}
                  label={t("statisticsSection.surfaceLabel", { surface }) as string}
                  className="w-full"
                >
                  <Stat
                    variant="rule"
                    label={t("statisticsSection.markets")}
                    value="17"
                    hint={t("statisticsSection.marketsHintShort")}
                  />
                  <Stat
                    variant="rule"
                    label={t("statisticsSection.employees")}
                    value="44,000"
                    hint={t("statisticsSection.employeesHint")}
                  />
                  <Stat
                    variant="rule"
                    label={t("statisticsSection.customers")}
        ...
        >
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                Appears on hover and on keyboard focus, and stays while pointed at.
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </Example>
        ...
        items={[
          {
            ...
            behaviour:
              "Progress exposes its value; the visible percentage is duplicated in the accessible name.",
          },
          {
            key: "Skeletons",
            behaviour:
          ...
          },
          {
            key: "Focus",
            behaviour:
              "Tooltips open on keyboard focus, not only on hover, and never contain interactive content.",
          },
        ]}
      />
    </CatalogSection>
